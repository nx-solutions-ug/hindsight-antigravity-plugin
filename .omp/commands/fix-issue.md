...
Make the **minimal, correct** code change that addresses the issue. Follow all project conventions from `rules/AGENTS.md`:

- Relative imports with `.js` extensions or module imports.
- Node built-ins use `node:` prefix (`node:fs`, `node:path`, `node:os`, `node:url`).
- Strict TypeScript compliance, no `as any` casts.
- No `console.log` in production code (use stderr for hook logging).
- Maintain Hindsight integration logic, hook contracts, and bank scoping.
...
Place tests in the appropriate location under `test/`. Follow existing test patterns found in Step 5. Use the same import conventions and test utilities as existing tests.
...
If any gate fails, fix the failure and re-run that gate. Do **not** proceed to Step 9 until both gates pass cleanly.
...
- <Test 1>
- <Test 2>
- All quality gates pass (build, tests)"
```
...
- **MUST** follow all project conventions from `rules/AGENTS.md`.
...
- **MUST** install dependencies with `bun install` (Step 0) before running any quality gates.
