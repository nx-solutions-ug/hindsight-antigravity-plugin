...
import { HOOK_WIRING, PLUGIN_NAME, RUNTIME_PACKAGE, SKILL_NAME } from '../src/host.js';
...
describe('bin wrappers', () => {
  ...
  test('import a build output the bundler actually produces', () => {
    ...
    for (const wrapper of wrappers) {
      ...
      // tsup names the entry bare (`name:`) or quoted (`'name':` / `"name":`).
      expect(tsup).toMatch(new RegExp(`['"]?${entry}['"]?\\s*:`));
    }
  });
  ...
  test('install.js imports only the installer bundle the build produces', () => {
    ...
    expect(source).toMatch(/from\s+['"]\.\.\/dist\/installer\.js['"]/);
